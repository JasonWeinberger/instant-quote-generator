...
export const PaymentSuccessPage: React.FC<PaymentSuccessPageProps> = ({ user, onActivate }) => {
  ...
  // 🔥 Fire Facebook Purchase event when payment is confirmed
  useEffect(() => {
    ...
    try {
      if (typeof window !== 'undefined' && (window as any).fbq) {
        (window as any).fbq('track', 'Purchase', {
          value: 29.0,      // <-- Change this if your price is different
          currency: 'USD',  // <-- Or 'CAD' etc.
        });
        console.log('[PaymentSuccessPage] fbq Purchase event fired');
      } else {
        console.log('[PaymentSuccessPage] fbq not found on window, Purchase not fired');
      }
    } catch (e) {
      console.error('[PaymentSuccessPage] Error firing fbq Purchase event', e);
    }
  }, [status]);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 animate-fade-in-up">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center border border-slate-200">
          ...
          <p className="text-slate-500 mb-6 text-sm leading-relaxed">
            ...
            <br />
            <br />
            Please check your inbox (and spam folder) to verify your email and access your Pro
            account.
          </p>
          <a
            href="/"
